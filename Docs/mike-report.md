...
Use a parameterized query, validate `userid` as an integer, protect the route with `verifyToken`, and remove `password` from the `SELECT`.
...
| Postman / Bruno | Sent structured requests with varying `userid` payloads |
...
| Bruno / Postman | Triggered the failing game creation request |
...
## Finding 5 - Merged into Finding 2

The game-data XSS sink is grouped into Finding 2 because it shares the same root cause: unescaped `innerHTML` rendering of attacker-controlled content. The same remediation applies to the review, game, category, and platform inputs listed in Finding 2.
